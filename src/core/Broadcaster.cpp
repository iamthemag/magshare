...
//
// $Id: Broadcaster.cpp 1371 2024-04-20 09:16:57Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
{
  if( !mp_broadcastSocket->bind( Settings::instance().hostAddressToListen(), static_cast<quint16>(Settings::instance().defaultBroadcastPort()), QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint ) )
  {
    qWarning() << "Broadcast receiver cannot bind the broadcast port" << Settings::instance().defaultBroadcastPort();
    return false;
  }

  connect( mp_broadcastSocket, SIGNAL( readyRead() ), this, SLOT( readBroadcastDatagram() ) );
...
{
  disconnect( mp_broadcastSocket, SIGNAL( readyRead() ), this, SLOT( readBroadcastDatagram() ) );
...
bool Broadcaster::broadcastToNetworkAddress( const NetworkAddress& na )
{
  ...
  if( mp_broadcastSocket->writeDatagram( broadcast_data, na.hostAddress(), host_port ) > 0 )
  {
...
void Broadcaster::readBroadcastDatagram()
{
  ...
  while( mp_broadcastSocket->hasPendingDatagrams() && num_datagram_read < MAX_NUM_OF_LOOP_IN_CONNECTON_SOCKECT )
  {
    num_datagram_read++;
    ...
    datagram.resize( static_cast<int>(mp_broadcastSocket->pendingDatagramSize()) );

    if( mp_broadcastSocket->readDatagram( datagram.data(), datagram.size(), &sender_ip, &sender_port ) == -1 )
    {
...
QList<NetworkAddress> Broadcaster::updateAddressesToSearchUsers()
{
  ...
  // Always add broadcast addresses for better network compatibility
  addHostAddress( NetworkManager::instance().localBroadcastAddress() );
  ...
  qDebug() << "MagShare: Force-adding broadcast address" << qPrintable( NetworkManager::instance().localBroadcastAddress().toString() );

  foreach( QHostAddress ha, NetworkManager::instance().localBroadcastAddresses() )
  {
    if( !ha.isNull() && ha != NetworkManager::instance().localBroadcastAddress() )
    {
      addHostAddress( ha );
