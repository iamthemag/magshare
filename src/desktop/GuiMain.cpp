...
//
// $Id: GuiMain.cpp 1567 2023-04-05 07:01:29Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
 ...
 : QMainWindow( parent ), m_floatingChats()
{
  ...
  setWindowIcon( IconManager::instance().icon( "magshare.png" ) );
  ...
  mp_tabMain->setDocumentMode( true );  // Modern tab appearance
  mp_tabMain->setUsesScrollButtons( true );
  ...
  mp_barMain->setToolButtonStyle( Qt::ToolButtonIconOnly );  // Icons only, no text
  mp_barMain->setIconSize( QSize(24, 24) );  // Standard icon size
  mp_barMain->setMovable( false );  // Modern fixed toolbar
  mp_barMain->setFloatable( false );
  mp_barMain->toggleViewAction()->setVisible( false );
  // Add spacing for modern layout
  ...
  // updadePluginMenu();  // Plugins menu removed from UI
  ...
  connect( mp_chatList, SIGNAL( removeUnsentMessagesInChat( VNumber ) ), this, SLOT( removeUnsentMessageInChat( VNumber ) ) );
...
void GuiMain::showAbout()
{
  ...
  QMessageBox::about( this, Settings::instance().programName(),
                      ...
                      QString( "<b>%1</b> - Messenger for connected Communities<br><br>%2<br>%3 %4 %5<br><br>Fork of: beebeep.net<br><br>%6 %7<br>%8<br>" )
                        ...
                        .arg( tr( "developed by" ) )
                        .arg( QString( "<a href='https://github.com/iamthemag'>Abdul Gafoor Mohammed</a>" ) )
                        .arg( QString( "e-mail: <a href='mailto://abdulgafoormohmd@gmail.com'>abdulgafoormohmd@gmail.com</a><br>web: <a href='https://github.com/iamthemag'>https://github.com/iamthemag</a>" ) )
                      );
}
...
void GuiMain::createActions()
{
  ...
  mp_actAbout = new QAction( IconManager::instance().icon( "magshare.png" ), tr( "About %1..." ).arg( Settings::instance().programName() ), this );
  ...
  mp_actViewNewMessage = new QAction( IconManager::instance().icon( "magshare-message.png" ), tr( "Show new message" ), this );
...
// #ifdef BEEBEEP_USE_SHAREDESKTOP - Desktop sharing feature disabled from UI
  ...
  act = mp_menuTrayIcon->addAction( IconManager::instance().icon( "magshare.png" ), tr( "Show" ), this, SLOT( showUp() ) );
...
void GuiMain::createToolAndMenuBars()
{
  ...
  QString label_tooltip = QString( "MagShare %1 %2" ).arg( Settings::instance().version( true, true, true ), Settings::instance().operatingSystem( true ) );
  ...
  // Default toolbar configuration - icons only
  ...
  // Ensure all toolbar buttons show icons only
  mp_barMain->setToolButtonStyle( Qt::ToolButtonIconOnly );
  QList<QToolButton*> toolButtons = mp_barMain->findChildren<QToolButton*>();
...
void GuiMain::showVCard( VNumber user_id )
{
  ...
  QRect screen_rect = qApp->screenAt( cursor_pos )->availableGeometry();
...
void GuiMain::showSavedChatSelected( const QString& chat_name )
{
  ...
  saved_chat->resize( qMin( (beeApp->primaryScreen()->availableGeometry().width()-80), 680 ), 420 );
...
{
  if( QMessageBox::question( qApp->activeWindow(), Settings::instance().programName(), tr( "Do you want to delete saved chat with %1?" ).arg( chat_name ), tr( "Yes" ), tr( "No" ), QString(), 1, 1 ) == 1 )
    return;
...
void GuiMain::removeUnsentMessageInChat( VNumber chat_id )
{
  Chat c = ChatManager::instance().chat( chat_id );
  if( !c.isValid() )
  {
  ...
  QString question_txt = tr( "Do you want to delete all unsent messages in the chat with %1?" ).arg( c.name() );
  int answer_id = QMessageBox::question( activeWindow(), Settings::instance().programName(), question_txt, tr( "Yes" ), tr( "No" ), QString(), 1, 1 );
  if( answer_id == 1 )
    return;

  beeCore->removeUnsentMessagesInChat( chat_id );
}
...
void GuiMain::loadUserStatusRecentlyUsed()
{
  ...
  std::sort( user_status_list.begin(), user_status_list.end() );
...
void GuiMain::onTickEvent( int ticks )
{
  ...
  {
    QIcon new_message_blinking_icon = IconManager::instance().icon( "magshare-message.png" );
    ...
    setWindowIcon( ticks % 2 == 0 ? new_message_blinking_icon : IconManager::instance().icon( "magshare.png" )  );
  }
...
void GuiMain::updateNewMessageAction()
{
  ...
  if( mp_actViewNewMessage->isEnabled() )
  {
    ...
    mp_tabMain->setTabIcon( chat_tab_index, IconManager::instance().icon( "magshare-message.png" ) );
...
void GuiMain::resetGeometryAndState()
{
  if( mp_dockFileTransfers->isVisible() )
    mp_dockFileTransfers->hide();
  resize( qMax( width(), 280 ), qMin( 520, qMax( beeApp->primaryScreen()->availableGeometry().height() - 120, 460 ) ) );
...
void GuiMain::showFileSharingWindow()
{
  ...
  if( !mp_fileSharing )
  {
    ...
    mp_fileSharing->resize( qMin( (beeApp->primaryScreen()->availableGeometry().width()-20), 860 ), 460 );
...
void GuiMain::showLogWindow()
{
  if( !mp_log )
  {
    ...
    mp_log->resize( qMin( (beeApp->primaryScreen()->availableGeometry().width()-20), 760 ), 460 );
...
void GuiMain::onShareDesktopImageAvailable( const User& u, const QImage& img, const QString& image_type, QRgb diff_color )
{
  ...
  int desktop_h = beeApp->primaryScreen()->availableGeometry().height() - 50;
  int desktop_w = beeApp->primaryScreen()->availableGeometry().width() - 80;
...
void GuiMain::loadStyle()
{
  ...
  // Apply modern 2025 stylesheet
  QString modernStylesheet = 
    "/* Modern 2025 UI Style for MagShare */\n"
    "QMainWindow {\n"
  ...
  {
    qDebug() << "Stylesheet: Modern Dark 2025";
    // Apply dark theme modifications to the modern stylesheet
    modernStylesheet = modernStylesheet
      .replace("#f8f9fa", "#2d3748")
      .replace("#212529", "#e2e8f0")
      .replace("#ffffff", "#1a202c")
      .replace("#f1f3f4", "#2d3748")
      .replace("white", "#1a202c")
      .replace("#e1e4e8", "#4a5568");
    beeApp->setPalette( Bee::darkPalette() );
  }
  else
  {
    qDebug() << "Stylesheet: Modern Light 2025";
    beeApp->resetPalette();
  }

  // Apply the modern stylesheet
  beeApp->setStyleSheet( modernStylesheet );
