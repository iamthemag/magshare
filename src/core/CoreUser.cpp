...
//
// $Id: CoreUser.cpp 1560 2023-03-15 07:56:27Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
bool Core::removeOfflineUser( VNumber user_id )
{
  ...
  if( UserManager::instance().removeUser( u ) )
  {
    ...
    emit userRemoved( u );
    int unsent_messages = MessageManager::instance().removeUnsentMessages( u.id(), ID_INVALID );
    if( unsent_messages > 0 )
      qDebug() << unsent_messages << "unsent messages to user" << u.name() << "deleted";
