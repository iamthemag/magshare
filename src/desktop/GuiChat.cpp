...
//
// $Id: GuiChat.cpp 1371 2024-04-20 09:16:57Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
void GuiChat::setupToolBar( QToolBar* chat_bar )
{
  ...
  /* Spell checking and word completer removed from toolbar but actions still initialized */
  mp_actSpellChecker = new QAction( IconManager::instance().icon( "spellchecker.png" ), tr( "Spell checking" ), this );
  connect( mp_actSpellChecker, SIGNAL( triggered() ), this, SLOT( onSpellCheckerActionClicked() ) );
  mp_actSpellChecker->setCheckable( true );
  mp_actCompleter = new QAction( IconManager::instance().icon( "dictionary.png" ), tr( "Word completer" ), this );
  connect( mp_actCompleter, SIGNAL( triggered() ), this, SLOT( onCompleterActionClicked() ) );
...
void GuiChat::updateActions( const Chat& c, bool is_connected, int connected_users, bool file_transfer_is_active )
{
  ...
  if( Settings::instance().disableSendFolder() )
    mp_actSendFolder->setEnabled( false );
  else
