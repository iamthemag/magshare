...
//
// $Id: BeeUtils.cpp 1566 2023-03-26 08:40:31Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
bool Bee::isLocalFile( const QUrl& file_url )
{
  ...
  if( file_path.size() > 2 && file_path.at( 1 ) == QChar( ':' ) )
    return true;
...
QPixmap Bee::convertToGrayScale( const QPixmap& pix )
{
  ...
  if( pixels*static_cast<int>( sizeof( QRgb ) ) <= img.sizeInBytes() )
  {
...
bool Bee::folderIsWriteable( const QString& folder_path, bool create_folder_if_not_exists )
{
  ...
  {
    bool ok = test_file.write( QByteArray( "MagShare" ) ) > 0;
    test_file.close();
