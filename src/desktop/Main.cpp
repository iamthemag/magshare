...
//
// $Id: Main.cpp 1566 2023-03-26 08:40:31Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
int main( int argc, char *argv[] )
{
  ...
  // Workaround for ping lag:
  // https://bugreports.qt.io/browse/QTBUG-40332
  qputenv("QT_BEARER_POLL_TIMEOUT", QByteArray::number(-1));

  // Enable high DPI scaling for all platforms for modern UI scaling
  QCoreApplication::setAttribute( Qt::AA_EnableHighDpiScaling, true );
  QCoreApplication::setAttribute( Qt::AA_UseHighDpiPixmaps, true );
  
  // Set scale factor for modern zoomed UI (1.2x scaling)
  qputenv("QT_SCALE_FACTOR", "1.2");

  BeeApplication bee_app( argc, argv );
  
  // Apply modern font with increased size for better readability
  QFont modernFont = bee_app.font();
  modernFont.setPointSize( modernFont.pointSize() + 2 );  // Increase base font size
  modernFont.setFamily("Segoe UI, -apple-system, BlinkMacSystemFont, sans-serif");  // Modern system font
  bee_app.setFont( modernFont );
  ...
  bee_app.setWindowIcon( QIcon( ":/images/magshare.png" ) );
  ...
  /* Load Settings */
  qDebug() << "Starting MagShare" << qPrintable( Settings::instance().version( true, false, true ) )
