...
//
// $Id: User.cpp 1560 2023-03-15 07:56:27Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
User::User()
 : m_id( ID_INVALID ), m_vCard(), m_networkAddress(), m_status( User::Offline ),
   ...
   m_workgroups(), m_localHostName( "" ), m_isHelper( false )
{
}
...
User::User( VNumber new_id, const UserRecord& ur )
 : m_id( new_id ), m_vCard(), m_networkAddress( ur.networkAddress() ), m_status( User::Offline ),
   ...
   m_workgroups(), m_localHostName( ur.localHostName() ), m_isHelper( false )
{
  if( !ur.name().isEmpty() )
    setName( ur.name() );
