...
//
// $Id: Protocol.cpp 1564 2023-03-22 10:59:00Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
Message Protocol::toMessage( const QByteArray& byte_array_data, int proto_version ) const
{
  ...
  Message m;
  QStringList sl = message_data.split( PROTOCOL_FIELD_SEPARATOR, Qt::KeepEmptyParts );
  if( sl.size() < 7 )
  {
...
QStringList Protocol::workgroupsFromHelloMessage( const Message& hello_message ) const
{
  ...
  if( workgroups.isEmpty() )
    ...
    return workgroups.split( ", ", Qt::SkipEmptyParts );
}
...
User Protocol::createUser( const Message& hello_message, const QHostAddress& peer_address )
{
  ...
  if( !sl.isEmpty() )
  {
    ...
    {
      QStringList user_workgroups_tmp = s_workgroups.split( ", ", Qt::SkipEmptyParts );
      if( Settings::instance().acceptConnectionsOnlyFromWorkgroups() && !Settings::instance().localUser().workgroups().isEmpty() )
      {
        ...
        user_workgroups = s_workgroups.split( ", ", Qt::SkipEmptyParts );
    }
  }
...
{
  QStringList sl = group_data_saved.split( DATA_FIELD_SEPARATOR, Qt::KeepEmptyParts );
  if( sl.size() < 5 )
    return Group();
...
QList<FileInfo> Protocol::messageFolderToInfoList( const Message& m, const QHostAddress& server_address, QString* pFolderName ) const
{
  ...
  sl = m.text().split( PROTOCOL_FIELD_SEPARATOR, Qt::SkipEmptyParts );
...
QList<FileInfo> Protocol::messageToShareBoxFileList( const Message& m, const QHostAddress& server_address ) const
{
  ...
  ShareDesktopData Protocol::imageDataFromShareDesktopMessage( const Message& m ) const
  {
    ...
    if( !m.data().isEmpty() )
    {
      QStringList sl_data = m.data().split( DATA_FIELD_SEPARATOR, Qt::KeepEmptyParts );
      if( sl_data.size() >= 4 )
      {
...
QList<UserRecord> Protocol::hiveMessageToUserRecordList( const Message& m ) const
{
  ...
  QStringList sl = m.text().split( PROTOCOL_FIELD_SEPARATOR, Qt::SkipEmptyParts );
  UserRecord ur;
...
#else
// FIXME: test if QRegularEpression pattern is the same of QRegExp
  linkfied_text.replace( QRegularExpression( "(((f|ht){1}tp(s:|:){1}//)[-a-zA-Z0-9@:%_\\+.,~#?!&//=\\(\\)]+)" ), "<a href=\"\\1\">\\1</a>" );
  linkfied_text.replace( QRegularExpression( "([\\s()[{}])(www.[-a-zA-Z0-9@:%_\\+.,~#?!&//=\\(\\)]+)" ), "\\1<a href=\"http://\\2\">\\2</a>" );
  linkfied_text.replace( QRegularExpression( "([_\\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\\.)+[a-zA-Z]{2,3})" ), "<a href=\"mailto:\\1\">\\1</a>" );
#endif
...
#else
// FIXME: test if QRegularEpression pattern is the same of QRegExp
  text_formatted.replace( QRegularExpression("(^|\\s|>)_(\\S+)_(<|\\s|$)"), "\\1<u>\\2</u>\\3" );
  text_formatted.replace( QRegularExpression("(^|\\s|>)\\*(\\S+)\\*(<|\\s|$)"), "\\1<b>\\2</b>\\3" );
  text_formatted.replace( QRegularExpression("(^|\\s|>)\\/(\\S+)\\/(<|\\s|$)"), "\\1<i>\\2</i>\\3" );
#endif
