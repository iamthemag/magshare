...
//
// $Id: MessageManager.cpp 1562 2023-03-20 16:17:34Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
QList<MessageRecord> MessageManager::takeMessagesToSendToUserId( VNumber user_id, bool also_sent_messages )
{
  ...
  {
    std::sort( message_list.begin(), message_list.end() );
    saveUnsentMessages( true );
  }
  return message_list;
}

int MessageManager::countMessagesToSendToUserId( VNumber user_id ) const
{
  int counter = 0;
  foreach( MessageRecord mr, m_messagesToSend )
  {
    if( user_id == ID_INVALID || mr.toUserId() == user_id )
      counter++;
  }
  return counter;
}

int MessageManager::countMessagesToSendInChatId( VNumber chat_id ) const
{
  int counter = 0;
  foreach( MessageRecord mr, m_messagesToSend )
  {
    if( chat_id == ID_INVALID || mr.chatId() == chat_id )
      counter++;
  }
  return counter;
}
...
int MessageManager::countUnsentMessages( VNumber to_user_id, VNumber chat_id ) const
{
  int num_messages = 0;
  QList<MessageRecord>::const_iterator it = m_sentMessages.begin();
  while( it != m_sentMessages.end() )
  {
    if( (to_user_id == ID_INVALID || (*it).toUserId() == to_user_id) && (chat_id == ID_INVALID || (*it).chatId() == chat_id) )
      num_messages++;
    ++it;
  }

  num_messages += countMessagesToSendToUserId( to_user_id );
  num_messages += countMessagesToSendInChatId( chat_id );

  return num_messages;
}

int MessageManager::removeUnsentMessages( VNumber to_user_id, VNumber chat_id )
{
  ...
  while( it != m_sentMessages.end() )
  {
    ...
    {
#ifdef MAGSHARE_DEBUG
      qDebug() << "Message manager removes unsent message from user" << to_user_id << "and chat" << chat_id;
#endif
      it = m_sentMessages.erase( it );
      messages_deleted++;
    }
