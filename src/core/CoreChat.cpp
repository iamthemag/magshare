...
//
// $Id: CoreChat.cpp 1560 2023-03-15 07:56:27Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
  ...
  foreach( User old_user, group_old_members.toList() )
  {
    if( !old_user.isLocal() )
    {
      if( !group_new_members.has( old_user.id() ) )
      {
        ...
        {
          MessageManager::instance().removeUnsentMessages( old_user.id(), c.id() );
...
bool Core::removeUserFromGroupChat( const User& u, const QString& chat_private_id )
{
  ...
  else
  {
    if( c.hasUser( u.id() ) )
    {
      MessageManager::instance().removeUnsentMessages( u.id(), c.id() );
...
bool Core::removeChat( VNumber chat_id, bool save_chat_messages )
{
  ...
  MessageManager::instance().removeUnsentMessages( ID_INVALID, c.id() );
  ...
  if( c.isGroup() )
  {
    dispatchSystemMessage( ID_DEFAULT_CHAT, ID_LOCAL_USER, tr( "%1 You have left group chat %2." )
                           .arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ), QString( "<b>%1</b>" ).arg( Bee::replaceHtmlSpecialCharacters( c.name() ) ) ),
  ...
  if( messages_sent > 0 )
  {
    foreach( VNumber ci, chat_list )
    {
      dispatchSystemMessage( ci, u.id(), QString( "%1 %2" )
                             .arg( IconManager::instance().toHtml( "unsent-message.png", "*m*" ),
                             tr( "Offline messages sent to %1." ).arg( Bee::userNameToShow( u, true ) ) ),
                             DispatchToChat, ChatMessage::Other, false );
    }
...
void Core::removeUnsentMessagesInChat( VNumber chat_id )
{
  int unsent_messages_deleted = MessageManager::instance().removeUnsentMessages( ID_INVALID, chat_id );
  dispatchSystemMessage( chat_id, ID_INVALID, QString( "%1 %2" )
    .arg( IconManager::instance().toHtml( "unsent-message.png", "*m*" ), tr( "%1 unsent message(s) deleted." ).arg( unsent_messages_deleted ) ),
          DispatchToChat, ChatMessage::Other, false );
  qDebug() << unsent_messages_deleted << "unsent messages deleted from chat" << chat_id;
}
...
void Core::addChatHeader( Chat* p_chat )
{
  ...
  if( p_chat->isDefault() )
  {
    // Startup copyright message removed for rebranded MagShare
