...
//
// $Id: CoreConnection.cpp 1564 2023-03-22 10:59:00Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
        ...
        else
        {
          if( u.name() == user_found.name() )
          {
            sAlertMsg = tr( "%1 Connection closed to user %2 because it uses same nickname of the already connected user %3: %4." )
                          .arg( IconManager::instance().toHtml( "warning.png", "*E*" ), Bee::replaceHtmlSpecialCharacters( u.path() ), Bee::replaceHtmlSpecialCharacters( user_found.path() ), Bee::userNameToShow( u, true ) );
    ...
    if( Settings::instance().disableConnectionSocketEncryption() )
    {
      dispatchSystemMessage( ID_DEFAULT_CHAT, u.id(),
                             QString( "%1 %2." ).arg( IconManager::instance().toHtml( "encryption-enabled.png", "*!*" ),
                                                      tr( "%1 has end-to-end encryption enabled" ).arg( Bee::userNameToShow( u, true ) ) ),
                             DispatchToAllChatsWithUser, ChatMessage::Connection, false );
    }
  }
  else
    ...
    if( !Settings::instance().disableConnectionSocketEncryption() )
    {
      dispatchSystemMessage( ID_DEFAULT_CHAT, u.id(),
                             QString( "%1 %2." ).arg( IconManager::instance().toHtml( "encryption-disabled.png", "*!*" ),
                                                      tr( "%1 has end-to-end encryption disabled" ).arg( Bee::userNameToShow( u, true ) ) ),
                             DispatchToAllChatsWithUser, ChatMessage::Connection, false );
    }
  }
