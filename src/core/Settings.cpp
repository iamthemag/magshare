...
//
// $Id: Settings.cpp 1371 2024-04-20 09:16:57Z mastroddi $
//
//////////////////////////////////////////////////////////////////////
...
{
  m_settingsVersion = MAGSHARE_SETTINGS_VERSION;
  m_currentFilePath = "";
...
#ifdef MAGSHARE_DISABLE_FILE_TRANSFER
...
#endif
#ifdef MAGSHARE_DISABLE_SEND_MESSAGE
...
#endif
#ifdef Q_OS_UNIX
  system_folders.append( QLatin1String( "/etc/magshare/" ) );
  system_folders.append( QLatin1String( "/usr/local/etc/magshare/" ) );
  system_folders.append( QLatin1String( "/usr/lib/magshare/" ) );
  system_folders.append( QLatin1String( "/usr/local/lib/magshare/" ) );
#endif
...
#ifdef Q_OS_UNIX
  data_folders.append( QLatin1String( "/usr/share/magshare/" ) );
  data_folders.append( QLatin1String( "/usr/local/share/magshare/" ) );
#endif
...
{
  QFileInfo rc_file_info( dataFolder() + Bee::nativeFolderSeparator() + QLatin1String( "magshare.rc" ) );
  ...
  if( sets->isWritable() )
  {
    ...
    sets->setValue( "DisableSendFolder", m_disableSendFolder );
...
{
  QString rc_file_path = findFileInFolders( QLatin1String( "magshare.rc" ), resourceFolders() );
...
#endif
  m_disableSendFolder = sets->value( "DisableSendFolder", m_disableSendFolder ).toBool();
...
bool Settings::createDefaultHostsFile()
{
  ...
  sl << "# This is a sample HOSTS file used by MagShare.";
  sl << "#";
  sl << "# This file contains the IP addresses (something like 10.0.0.123)";
  sl << "# or subnet (10.0.0.255 means that MagShare tries to connect every";
...
bool Settings::isDevelopmentVersion( const QString& v ) const
{
  QString s_version;
  if( v.isEmpty() )
    s_version = QString( MAGSHARE_VERSION );
  else
    s_version = v;
  ...
  if( sl.isEmpty() )
  {
    qWarning() << "INVALID MAGSHARE VERSION FOUND";
    return true;
  }
...
{
  QString s_version = QString( MAGSHARE_VERSION );
  if( build_version )
    s_version += QString( "-%1" ).arg( MAGSHARE_BUILD );

  if( debug_info )
  {
    s_version += QString( "-p%2" ).arg( MAGSHARE_PROTO_VERSION );
#ifdef MAGSHARE_DISABLE_FILE_TRANSFER
...
{
  return QString( "%1 %2" ).arg( programName(), version( false, false, false ) );
}

int Settings::protocolVersion() const
{
  return MAGSHARE_PROTO_VERSION;
}
...
{
  return QString( MAGSHARE_NAME );
}

QString Settings::organizationName() const
{
  return QString( MAGSHARE_ORGANIZATION );
}

QString Settings::organizationDomain() const
{
  return QString( MAGSHARE_ORGANIZATION_DOMAIN );
}

QString Settings::officialWebSite() const
{
  return QString( MAGSHARE_WEBSITE );
}

QString Settings::pluginWebSite() const
{
  return officialWebSite() + QString( MAGSHARE_DOWNLOAD_WEBSITE ) + "#plugin";
}

QString Settings::downloadWebSite() const
{
  return officialWebSite() + QString( MAGSHARE_DOWNLOAD_WEBSITE );
}

QString Settings::donationWebSite() const
{
  return officialWebSite() + QString( MAGSHARE_DONATE_WEBSITE );
}
...
{
  return officialWebSite() + QString( MAGSHARE_LANGUAGE_WEBSITE );
}

QString Settings::lastVersionUrl() const
{
  return officialWebSite() + QString( MAGSHARE_LAST_VERSION_FILE );
}
...
{
  return QString( "https://github.com/iamthemag" );
}
...
QString Settings::operatingSystem( bool use_long_name ) const
{
  ...
  os_name_short = "OS2";
#endif
#ifdef MAGSHARE_FOR_RASPBERRY_PI
...
{
  QString os_icon_path = "magshare.png";
...
QString Settings::checkVersionWebSite() const
{
  ...
  return officialWebSite() + QString( "%1?magshare-version=%2&magshare-os=%3" ).arg( QString( MAGSHARE_CHECK_VERSION_WEBSITE ) ).arg( QString( MAGSHARE_VERSION ) ).arg( os_type );
}
...
{
  return QString( MAGSHARE_DNS_RECORD );
}

QString Settings::gaTrackingId() const
{
  return QString( MAGSHARE_GA_TRACKING_ID );
}

QString Settings::gaUrl() const
{
  return QString( MAGSHARE_GA_URL );
}

QString Settings::gaEventVersion() const
{
  return QString( MAGSHARE_GA_EVENT_VERSION );
}
...
void Settings::load()
{
  ...
  m_settingsVersion = sets->value( "Settings", MAGSHARE_SETTINGS_VERSION ).toInt();
...
void Settings::loadCommonSettings( QSettings* user_ini )
{
  ...
  m_disableTabInEditBox = commonValue( system_rc, user_ini, "DisableTabInEditBox", m_disableTabInEditBox ).toBool();
...
void Settings::save()
{
  ...
  sets->setValue( "Settings", MAGSHARE_SETTINGS_VERSION );
  ...
  sets->setValue( "DisableTabInEditBox", m_disableTabInEditBox );
  sets->endGroup();
...
bool Settings::searchDataFolder()
{
  ...
  QString data_folder = m_addAccountNameToDataFolder ? Bee::removeInvalidCharactersForFilePath( accountNameFromSystemEnvinroment() ) : QLatin1String( "magshare-data" );
  QString root_folder;
#ifdef Q_OS_MACOS
  ...
  bool rc_folder_is_writable = Bee::folderIsWriteable( m_resourceFolder, false );
    ...
    if( !env_program_files.isEmpty() )
      qDebug() << "Checking if MagShare is installed in system folder:" << qPrintable( env_program_files ) << "...";
    ...
    {
      qDebug() << "MagShare is installed in system folder:" << qPrintable( m_resourceFolder );
      qDebug() << "Resource folder is default Windows Program Files and will not be used as data folder";
      rc_folder_is_writable = false;
    }
    else
      qDebug() << "MagShare is installed in custom folder:" << qPrintable( m_resourceFolder );
  #endif
#endif
...
bool Settings::setDataFolder()
{
  if( !rcFileExists() )
  {
    ...
    m_dataFolder = findFileInFolders( "magshare.ini", default_data_folders, true );
  }
  else
    m_dataFolder = "";
...
{
  return Bee::convertToNativeFolderSeparator( QString( "%1/%2" ).arg( dataFolder(), QLatin1String( "magshare.dat" ) ) );
}

QString Settings::autoSavedChatsFilePath() const
{
  return Bee::convertToNativeFolderSeparator( QString( "%1/%2" ).arg( backupFolder(), QLatin1String( "magshare.bak" ) ) );
}

QString Settings::unsentMessagesFilePath() const
{
  return Bee::convertToNativeFolderSeparator( QString( "%1/%2" ).arg( dataFolder(), QLatin1String( "magshare.off" ) ) );
}

QString Settings::defaultSettingsFilePath() const
{
  return Bee::convertToNativeFolderSeparator( QString( "%1/%2" ).arg( dataFolder(), QLatin1String( "magshare.ini" ) ) );
}
...
QString Settings::defaultLanguageFolderPath() const
{
  ...
  data_folders.append( resourceFolders() );
  data_folders.removeDuplicates();
  QString test_language_file = QLatin1String( "magshare_it.qm" );
...
QString Settings::simpleDecrypt( const QString& text_to_decrypt )
{
  ...
  if( m_settingsVersion < 5 )
  {
    ...
    foreach( QChar c, text_to_decrypt )
      text_decrypted += QChar( c.toLatin1() ^ key );
  }
  else
  {
